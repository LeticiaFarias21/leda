...
public class HashtableClosedAddressImpl<T> extends
	...
	public void remove(T element) {
		if(element != null && !this.isEmpty()){
			if(this.indexOf(element) != -1){
				LinkedList<T> list = (LinkedList<T>) this.table[this.indexOf(element)];
				//Se tiver colisão diminuir
				if(list.size() > 1){
					this.COLLISIONS--;
				}
				list.remove(element);
	...
	public T search(T element) {
		if(((LinkedList<T>)this.table[hash(element)]).contains(element)){
			return element;
		}
		return null;
	}
	...
	public int indexOf(T element) {
		int index = -1;
		if(element != null && !this.isEmpty()){
			LinkedList<T> list = (LinkedList<T>) this.table[this.hash(element)];
			if(list != null && list.contains(element)){
				index = this.hash(element);
			}
		}
		return index;
	}
