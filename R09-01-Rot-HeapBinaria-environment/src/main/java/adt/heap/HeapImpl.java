...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		...
		if (position >= 0) {
			...
			if (left <= this.index && comparator.compare(heap[left], heap[position]) > 0) {
				largest = left;
			} else {
				largest = position;
			}
			if (right <= this.index && comparator.compare(heap[right], heap[largest]) > 0) {
				largest = right;
			}
			if (largest != position) {
	...
	public void insert(T element) {
		if(element != null) {
			...
			heap[++index] = element;
			int auxiliar = index;
			while(auxiliar > 0 && comparator.compare(heap[auxiliar], heap[parent(auxiliar)]) > 0){
				Util.swap(heap, auxiliar, parent(auxiliar));
		...
		if(array.length >= 2){
			...
			if(rootElement().compareTo(heap[index]) > 0){
				...
				for(int index = 0; index < array.length; index++){
					array[index] = extractRootElement();
				}
			}
		}
		return array;
